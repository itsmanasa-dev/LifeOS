  ...
  static List<TimetableEntry> parse(String text) {
    ...
    for (var line in lines) {
      ...
      if (timeMatch != null) {
        ...
        String rawStart = timeMatch.group(1)!;
        String rawEnd = timeMatch.group(2)!;

        // Correct common OCR character swaps
        String cleanStart = rawStart.replaceAll(RegExp(r'[oO]'), '0').replaceAll('.', ':');
        String cleanEnd = rawEnd.replaceAll(RegExp(r'[oO]'), '0').replaceAll('.', ':');

        if (cleanStart != rawStart) {
          lowConfidence.add('startTime');
        }
        if (cleanEnd != rawEnd) {
          lowConfidence.add('endTime');
        }

        final timeFormatRegex = RegExp(r'^\d{2}:\d{2}$');
        if (!timeFormatRegex.hasMatch(cleanStart)) {
          lowConfidence.add('startTime');
          if (cleanStart.length == 4 && RegExp(r'^\d:\d{2}$').hasMatch(cleanStart)) {
            cleanStart = '0$cleanStart';
          } else {
            cleanStart = '09:00';
          }
        }
        if (!timeFormatRegex.hasMatch(cleanEnd)) {
          lowConfidence.add('endTime');
          if (cleanEnd.length == 4 && RegExp(r'^\d:\d{2}$').hasMatch(cleanEnd)) {
            cleanEnd = '0$cleanEnd';
          } else {
            cleanEnd = '09:50';
          }
        ...
        if (subjectText.isEmpty) {
          ...
          lowConfidence.add('subjectName');
        } else if (subjectText.length < 3 || RegExp(r'^\d+$').hasMatch(subjectText)) {
          lowConfidence.add('subjectName');
        }

        if (!foundDay && !dayDetected) {
          lowConfidence.add('dayOfWeek');
        }
